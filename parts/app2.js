...
function hintFor(q){
  /* Підказка показує саме правильну відповідь — так її просила Вікторія.
     Пояснення правила, якщо воно є, лишається другим рядком.            */
  const a=answerOf(q);
  const rule=q.hint ? '<span class="hint__why">'+q.hint+'</span>' : '';
  const show=v=>'Правильна відповідь: <b>'+esc(v)+'</b>'+rule;
  switch(q.ex){
    case "artikel": case "artpl":
      return show(a+" "+String(q.say||"").replace(ARTS,""));
    case "build": case "listen":
      return show(String(q.ans));
    case "pairs":
      return q.pairs
        ? '<b>Пари:</b><span class="hint__why">'
          +q.pairs.map(p=>esc(p[0])+" — "+esc(p[1])).join("<br>")+'</span>'
        : show(a);
    default:
      return show(a);
  }
